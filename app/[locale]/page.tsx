    ...
    <>
      ...
      >
        ...
        <div className="mt-12 w-full xl:max-w-xl px-4">
          {!submitted ? (
            <form onSubmit={handleSubmit} className="flex flex-col md:grid grid-cols-6 gap-2 md:gap-5 py-2">
              <input
                required
                name={entryId}
                type="email"
              ...
              <button
                type="submit"
                className="bg-yellow-500 text-black font-thin text-xs md:text-base md:font-medium col-span-2 p-2 md:p-4 rounded-xl hover:bg-yellow-600 focus:ring-2 focus:ring-yellow-500"
              >
                {locale.addToWaitlist}
              </button>
            </form>
          ) : (
            <p className="text-green-500 text-lg">Thank you for subscribing!</p>
          )}
        </div>
        ...
        {/* Language Selector */}
        <Link href={locale.language} className="flex items-center text-white gap-2 mt-4 ">
          <Image src={globe} alt="Language" width={24} height={24} unoptimized />
